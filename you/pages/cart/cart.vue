<template>
	<view class="cartContainer">
		...
		<!-- block代表块级意思，标识多个元素是个整体，多用于判断，不会在页面渲染 -->
		...
		<block v-else>
			<block v-if="carList.length>0">
				<!-- 购物车列表 -->
				<view class="cartList">
					<view class="cartItem" v-for="(item,index) in carList" :key="item.id">
						<text
						 class='iconfont icon-xuanze selected'
						 ></text>
						<view class="shopItem">
							<image class="shopImg" :src="item.listPicUrl"></image>
							<view class="shopInfo">
								<text>{{item.name}}</text>
								<text class="price">{{item.retailPrice | currency}}</text>
							</view>
						</view>
						<!-- 控制数量 -->
						<view class="countCtrl">
							...
							<text class="del" @click="del(index)"> - </text>
						</view>
					</view>
				...
				<view class="cartFooter">
					...
					<view class="right">
						<text class="totalPrice">合计: {{sumPrice|currency}}</text>
